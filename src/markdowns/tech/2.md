...
As per <abbr title="Fish doc"><a href="https://fishshell.com/docs/current/design.html#configurability-is-the-root-of-all-evil"  target='_blank'>Fish's design</a></abbr>, it states <em>Configurability is the root of all evil</em>, and that a program should be "smart" enough to offer functionalities that avoids the trouble of users manually configuring it.
...
Fish's ootb syntax highlighting, again, got everything you need and you have plenty of themes to choose from. Every time you think about some symbol, or even a space between 2 directories in `cd` command, that warrant a syntax color change, Fish will do it. I've heard <em>zsh</em> also has some impressive syntax highlighting configurability. I did try it and it does indeed offer a ton of <em>configurabilities</em> but it does not give you a working one to work with (why?), and when I start implementing plugins to complement its highlight engine, I just ended up replicating Fish with only roughly <40% of what Fish can offer, and I've spent a great deal of time into it to achive that. 

## And many more features await discovering

Fish is a <em>HUGE</em> program that covers a seemingly insurmountable amount of features that wait for you to discover. I've only used what I needed to so I won't be able to even cover basic usage of Fish, if that even exists. I'll continue to ramble about Fish in the future once I discover more fun with it, stay tune.
